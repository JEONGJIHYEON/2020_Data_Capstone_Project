...
- α의 값이 점점 작아질수록 사용하는 feature의 수가 늘어남과 동시에 train set의 점수는 높아지는 반면, test set의 점수는 점차 감소함을 볼 수 있다. 즉, 과대적합의 문제가 커지고 있다고 할 수 있다. 
...
- **프로젝트의 결과를 개선**하기 위해서는 우선 데이터 수집 기간을 늘려(예. 1년 단위) 충분한 양의 데이터를 확보하는 것이 중요하다. 또한, 같은 상권 구역으로 묶어 단순히 feature와 총 매출 사이의 관계를 분석하는 기존의 방법을 넘어 분류 대상을 다른 변수로 잡는 등 이번 프로젝트의 방향과 다른 방법을 선택하면 지금보다 의미있는 결과를 얻을 수 있을 것이다. 예를 들어, 패스트푸드점, 카페, 독서실, 의류 매장 등 업종별로 묶어 각 업종의 총 매출에 영향을 미치는 요인을 분석하면 좀 더 뚜렷한 목표치에 대한 분석 결과를 얻을 수 있을 것이라 생각한다.
...
- 위와 같이 다양한 방법으로 정밀한 분석 과정을 거친다면 같은 데이터로도 정확하고 무궁무진한 결과를 도출할 수 있고 이를 통해 소상공인들에게 도움을 줄 수 있을 것으로 기대된다. 
